 ...
 */
export async function moveImageFromTempToPermanent(tempImageUrl: string): Promise<string> {
  try {
    ...
    console.log(`Image moved from temp to permanent: ${tempKey} -> ${permanentKey}`);
    
    return permanentUrl;
  } catch (error) {
    console.error(`Error moving image from temp to permanent: ${tempImageUrl}`, error);
